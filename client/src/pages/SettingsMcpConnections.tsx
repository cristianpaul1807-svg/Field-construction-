...
export default function SettingsMcpConnections() {
  ...
  return <div className="p-4 sm:p-8 space-y-6 max-w-5xl mx-auto">
    <PageHeader title="MCP Conexiones AI" description="Conecta Claude a los datos de Logiciel Construction que tu rol y tu plan permiten consultar." />
      ...
      {platforms.map((platform) => {
        ...
        return <Card key={platform.id} className="overflow-hidden">
          ...
          {isOpen && <div className="border-t border-border p-5 space-y-5">
            ...
            <div className="flex flex-wrap items-center gap-2"><Button type="button" size="sm" className="gap-1.5" onClick={() => window.open(platform.docsUrl, "_blank", "noopener,noreferrer")}><ExternalLink size={14} /> Abrir {platform.name}</Button><Button type="button" variant="outline" size="sm" className="gap-1.5" onClick={() => window.open(platform.docsUrl, "_blank", "noopener,noreferrer")}><ExternalLink size={14} /> Guía de {platform.name}</Button>{platform.connections.filter((connection) => connection.status !== "revoked").map((connection) => <Button key={connection.id} type="button" variant="ghost" size="sm" className="gap-1.5 text-destructive" onClick={() => revoke(connection.id)} disabled={revoking === connection.id}>{revoking === connection.id ? <Spinner className="size-3.5" /> : <Users size={14} />} Revocar conexión</Button>)}</div>
          </div>}
        </Card>;
      })}
    </div>}
